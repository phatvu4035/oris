odoo.define('oristar_ecommerce_website.main', function (require) {
    "use strict";

    const { Component, Store, mount, router, QWeb } = owl;
    const { whenReady } = owl.utils;
    const { useRef, useDispatch, useState, useStore } = owl.hooks;
    const ProductSearch = require('oristar_ecommerce_website.ProductSearch');
    ...
    async function setup() {
        owl.config.mode = "dev";
        var templates = await owl.utils.loadFile('/oristar_ecommerce_website/static/src/xml/search_product_components.xml');
